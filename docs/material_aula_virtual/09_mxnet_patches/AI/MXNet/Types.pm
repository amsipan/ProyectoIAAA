...
subtype "MaybeIndex"      => as "Maybe[Int]";
...
subtype "WholeDim"        => as "Str" => where { $_ =~ /^[X:]$/ };
subtype "Ellipsis"        => as "Str" => where { $_ eq '...' };
subtype "Slice"           => as "ArrayRef[MaybeIndex]|WholeDim|Index|Ellipsis" => where { ref $_ ? @$_ > 0 : 1 };
