...
method call(@args)
{
    ...
    my $out = [$self->forward(@args)]; # Originally: my @out = $self->forward(@args);
    for my $hook ($self->_forward_hooks->values)
    {
        $hook->($self, \@args, $out); # Originally: $hook->($self, \@args, \@out); discarding changes on @out variable.
    }
    return wantarray ? @$out : $out->[0]; # Originally: return wantarray ? @out : $out[0];
}
...
method forward(@args)
{
    confess("Method forward is not Implemented");
}
