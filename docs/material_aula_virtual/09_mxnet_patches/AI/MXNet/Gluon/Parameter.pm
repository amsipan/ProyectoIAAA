...
)
{
    ...
    #################### Code correction for aliasing weight|bias #####################
    my %arg_dict;
    # Check if this file contains flat legacy keys (e.g., '0weight', '0bias')
    my $is_legacy_flat = (grep { /^\d+(?:weight|bias)$/ } keys %orig_load) ? 1 : 0;

    if ($is_legacy_flat)
    {
        # Map the internal network keys to their legacy equivalents based on order/layer index
        # internal key example: "sequential0_dense0_weight"
        my @net_keys = $self->keys();
        
        for my $net_key (@net_keys)
        {
            # Extract the numeric index and param type (weight/bias) from the actual network key
            if ($net_key =~ /dense(\d+)_(weight|bias)$/)
            {
                my $legacy_key = $1 . $2; # e.g., "0" . "weight" = "0weight"
                if (exists $orig_load{$legacy_key})
                {
                    $arg_dict{$net_key} = $orig_load{$legacy_key};
                }
            }
            # Fallback for keys that might match directly
            elsif (exists $orig_load{$net_key})
            {
                $arg_dict{$net_key} = $orig_load{$net_key};
            }
        }
    }
    else
    {
        # Standard native mapping behavior for properly structured files
        %arg_dict = map { my $k = $_; s/^(?:arg|aux)://; ($restore_prefix.$_, $orig_load{$k}) } keys %orig_load;
    }
    #################### Code correction for aliasing weight|bias #####################  
