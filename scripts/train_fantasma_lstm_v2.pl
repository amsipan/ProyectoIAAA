...
# LSTM v2 anti-overfit: validacion en cola causal del train, early stopping
# por MAE promedio, dropout explicito antes del Dense y grid corto de runs.
# Artefactos con sufijo _v2 en out_dir; el baseline v1 queda intacto.
...
my %opt = (
    ...
    out_dir     => 'Data/ml_out/lstm_fantasma_v2',
    seq_len     => 5,
    batch_size  => 64,
    hidden      => 32,
    num_layers  => 1,
    ...
    seed        => 42,
    eval_only   => 0,
    model       => undef,     # default: out_dir/fantasma_lstm_v2.params
    binary_thr  => 1,
    grid        => undef,     # "name:hidden:dropout:lr:batch,..."
    run_name    => 'single',
);
...
$opt{model} //= "$opt{out_dir}/fantasma_lstm_v2.params";
...
if ( $opt{eval_only} ) {
    my $cfgfile = "$opt{out_dir}/train_config_v2.json";
    if ( -f $cfgfile ) {
        my $j = _slurp_json($cfgfile);
        if ( my $ch = $j->{chosen} ) {
            for my $k (qw( hidden num_layers dropout lr batch_size seq_len )) {
                next if $cli_given{$k};
                $opt{$k} = $ch->{$k} if defined $ch->{$k};
            }
            print "[*] eval-only hereda config elegida ($j->{chosen_run}): "
              . "hidden=$opt{hidden} dropout=$opt{dropout} seq_len=$opt{seq_len}\n";
        }
    }
}

print "[*] MXNet OK — Fantasma LSTM v2 (early stop + val causal + dropout denso)\n";
...
package FantasmaLSTMV2 {
    ...
    sub new {
        ...
        map { $self->register_child( $self->{$_} ) } ( 'lstm', 'drop', 'dense' );
...
# Confusion completa: pos = >=1 rastro real; pred>=0.5 -> pos (convencion baseline v1)
...
sub train_run {
    my ($cfg) = @_;
    printf "[*] === run %s: hidden=%d dropout=%.2f lr=%g batch=%d ===\n",
      $cfg->{name}, $cfg->{hidden}, $cfg->{dropout}, $cfg->{lr}, $cfg->{batch_size};
...
sub final_eval_and_write {
    ...
    print "[*] Evaluando test julio (modelo final v2)...\n";
    ...
    my $metrics_path = "$opt{out_dir}/metrics_test_v2.json";
    my $preds_path   = "$opt{out_dir}/preds_test_v2.csv";
    my $cfg_path     = "$opt{out_dir}/train_config_v2.json";
...
if ( defined $opt{grid} ) {
    for my $part ( split /,/, $opt{grid} ) {
        my @f = split /:/, $part;
        die "grid: formato name:hidden:dropout:lr:batch (recibi '$part')\n" unless @f == 5;
        push @cfgs, {
            name       => $f[0],
            hidden     => int( $f[1] ),
            dropout    => 0 + $f[2],
...
for my $r (@runs) {
    my $c = $r->{cfg};
    printf "[*] %-8s h=%-3d d=%.1f lr=%-5g b=%-3d epochs=%02d best_ep=%02d val_MAE=%.4f test_MAE\@best=%.4f%s\n",
      $c->{name}, $c->{hidden}, $c->{dropout}, $c->{lr}, $c->{batch_size},
...
# Comparacion corta contra v1 si sus metricas estan disponibles (solo lectura)
my $v1_metrics_path = 'Data/ml_out/lstm_fantasma/metrics_test.json';
if ( -f $v1_metrics_path ) {
    my $v1 = _slurp_json($v1_metrics_path);
    my ( $v1_mae, $v1_f1 ) = ( 0, 0 );
    $v1_mae += $v1->{regression}{$_}{mae} for @{ $v1->{targets} };
    $v1_mae /= scalar @{ $v1->{targets} };
    print "[*] === Comparacion vs v1 (test julio) ===\n";
    printf "[*] MAE avg:  v1=%.4f  v2=%.4f  (delta %+.4f)\n",
      $v1_mae, $metrics->{mae_avg}, $metrics->{mae_avg} - $v1_mae;
    for my $t ( @{ $v1->{targets} } ) {
        printf "[*] %s  MAE v1=%.4f v2=%.4f | RMSE v1=%.4f v2=%.4f\n",
          $t, $v1->{regression}{$t}{mae}, $metrics->{regression}{$t}{mae},
          $v1->{regression}{$t}{rmse}, $metrics->{regression}{$t}{rmse};
    }
}
...
sub print_usage {
    print <<'USAGE';
train_fantasma_lstm_v2.pl — LSTM v2 anti-overfit (val causal + early stop + grid)
  ...
  --out-dir PATH       (default Data/ml_out/lstm_fantasma_v2)
  --seq-len N          (default 5)
  --batch-size N       (default 64; run individual)
  --hidden N           (default 32; run individual)
  --num-layers N       (default 1)
  --dropout F          (default 0.2; run individual)
  --lr F               (default 0.01; run individual)
  ...
  --model PATH         (default OUT/fantasma_lstm_v2.params)
  --grid SPEC          varias configs: "r01:32:0.2:0.01:64,r02:16:0.3:0.005:32"
                       (name:hidden:dropout:lr:batch)
...
Entrena solo con sub-train; early stop por MAE promedio en cola causal del
train; guarda mejor checkpoint por run; el mejor run se copia al modelo final.
Reporta MAE/RMSE, bin_acc (>=thr) y confusion completa (pred>=0.5) en test.
USAGE
}
