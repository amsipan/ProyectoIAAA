...
# ===========================================================================
# Test 2: size('D') ~= días con datos, size('W') ~= semanas.
# ===========================================================================
$md->set_timeframe('D');
my $d_size = $md->size();
is($d_size, 2, 'D size = 2 (días 06 y 07 de abril)');

my $d0 = $md->get_candle(0);
is($d0->[0], '2026-04-06T00:00:00-05:00', 'D[0] bucket = 2026-04-06');
# Open del día = primera vela 1m del día = 100
is($d0->[1], 100, 'D[0] Open = 100');
# Close del día = última vela 1m del día 06 = (59+60)*10+102 = 1292
is($d0->[4], 1292, 'D[0] Close = 1292');
# Vol = suma de 180 velas... no, solo 120 velas del día 06
# suma(1..120) = 120*121/2 = 7260
...
# ===========================================================================
# Test 4: 2h y 4h agregan correctamente.
# ===========================================================================
$md->set_timeframe('2h');
# 2h: hora 0+1 del día 06 = 1 vela 2h, hora 0 del día 07 = 1 vela 2h
is($md->size(), 2, '2h size = 2');
my $h2_0 = $md->get_candle(0);
is($h2_0->[0], '2026-04-06T00:00:00-05:00', '2h[0] bucket = 00:00');
# Open = 100, Close = última vela 1m del bucket = 119*10+102 = 1292
is($h2_0->[1], 100, '2h[0] Open = 100');
is($h2_0->[4], 1292, '2h[0] Close = 1292');

$md->set_timeframe('4h');
# 4h: hora 0+1+2+3 del día 06 (solo tenemos 0+1) = 1 vela 4h parcial, hora 0 del 07 = 1 vela 4h
is($md->size(), 2, '4h size = 2');
my $h4_0 = $md->get_candle(0);
is($h4_0->[0], '2026-04-06T00:00:00-05:00', '4h[0] bucket = 00:00');
