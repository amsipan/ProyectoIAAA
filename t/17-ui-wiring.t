...
# =============================================================================
{
    ...
    like($src, qr/my %density_items = \(/, '0065: market.pl usa selector compacto de grupo/item para densidad');
    like($src, qr/my \$density_groups_box = \$density_rows_box->Frame\(\)->pack\(-side => 'top'/, '0066: densidad separa grupos en primera fila');
    like($src, qr/my \$density_items_box = \$density_rows_box->Frame\(\)->pack\(-side => 'top'/, '0066: densidad separa específicos en segunda fila');
    like($src, qr/-from\s*=>\s*1,/, '0066: slider visible de densidad empieza en 1');
    like($src, qr/-to\s*=>\s*100,/, '0065/0066: slider de densidad permite 100');
    like($src, qr/-length\s*=>\s*240,/, '0066: slider de densidad es mas largo para manipular');
    like($src, qr/my \$density_detail_row = \$frame_controles->Frame\(\)/, '0067: densidad usa fila de detalle desplegable');
    ...
    like($src, qr/set_element_density_pct/, '0063/0065: market.pl cablea set_element_density_pct por familia');
